					...
					{loggedIn && (
						<IconButton
							...
							classes={{
								...
								label: clsx(
									appbarClasses.accountIconButtonLabel,
									"navbar-foreground-shift"
								),
							}}
						>
...
const NotificationBanner = connect(_mapStateToProps)(
	({
		bannerStuff: { isOpen, variant, message, delay },
		user: {
			userSettings: { currentActiveKeys },
		},
	}) => {
		...
		return (
			<div
				className={clsx(
					classes.notificationBannerOuter,
			...
			>
				<div className={classes.bannerTextContainer}>
					{message &&
						[...message].map((ch, index) => {
							return (
								<Typography
									variant="h6"
									className={clsx(
										classes.bannerCharacter,
										"notification-banner-text"
									)}
									key={`notification-banner-${index}`}
								>
									{ch}
								</Typography>
							);
