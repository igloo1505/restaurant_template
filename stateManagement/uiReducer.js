	...
	switch (action.type) {
		...
		case Types.SET_ADD_RECIPE_STEP: {
			...
			if (action.payload === "increase") {
				_newStep =
					state.addRecipe.activeStep < 2 ? state.addRecipe.activeStep + 1 : 2;
			}
			if (action.payload === "decrease") {
				_newStep =
					state.addRecipe.activeStep > 0 ? state.addRecipe.activeStep - 1 : 0;
			}
		...
		case Types.TOGGLE_SET_KEYS_BACKDROP: {
			return {
				...
				settingsModal: {
					...
					...(action?.payload?.settingKeysBackdrop && {
						settingKeysBackdrop: action.payload.settingKeysBackdrop,
					}),
